...
public class Damage : MonoBehaviour {
	...
	void OnTriggerEnter(Collider collision)						// used for things like bullets, which are triggers.  
	{
		if (damageOnTrigger) {
			...
			if (collision.transform.root.gameObject.GetComponent<Health> () != null) {	// if the hit object has the Health script on it, deal damage
				collision.transform.root.gameObject.GetComponent<Health> ().ApplyDamage (damageAmount);
